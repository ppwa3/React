...
const countReducer = (prevCount, action) => {
  /** 
   매개변수로 전달된 Action객체를 분석해서 스테이트를 변경한 후 반환한다.
...
function App() {
  ...
  */
 /*  countDispatch(action(객체))가 호출되면
 ex. countReducer(count=prevCount, { mode: 'up', number: 3 }=action) */
  const [count, countDispatch] = useReducer(countReducer, 0);
  //입력상자의 증가치 변경을 위한 스테이트와 함수정의
