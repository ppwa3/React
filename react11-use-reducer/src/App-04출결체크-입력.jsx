  ...
  return(<>
    <div>
      ...
      <span style={{textDecoration: isHere ? 'line-through' : 'none',
        ...
        onClick={()=>{
          /*
          디스패치 함수 호출시 액션 객체를 전달해서 상태 변경. 이때
          리듀서 함수가 실행된다.
          */
...
const reducer = (state, action) => {
  ...
  switch(action.type){
    // 학생 추가
    case 'add':
      // 학생이름을 Action객체를 통해 읽어온다. (dispatch함수에서 전달하는 액선객체 참조)
    ...
    case 'delete':
      return{
        count: state.count - 1,
        students: state.students.reduce((pv,cv)=>{
            if(cv.id !== action.param.id){
              pv.push(cv);
            }
            return pv;
          }, []),
        /*
        filter함수를 이용해서 삭제할 학생을 제외한 나머지 객체를 반환하도록
        정의한 후 반환값을 통해 업데이트한다.
