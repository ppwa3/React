/* react-router-dom으로부터 임포트한 컴포넌트와 관련 훅 */
...
const TopNavi = () => {
  /* NavLink 컴포넌트는 a태그와 같이 하이퍼링크를 제공한다.
    단, a태그에 preventDefault()가 적용된 형태로 화면의 깜빡임없이 페이지 이동을
    할 수 있다. 또한 링크를 클릭했을때 active라는 클래스 속성을 자동으로 추가해준다. */
  return (<>
    <nav>
      ...
      {/* a태그를 사용하는 경우 화면의 깜박임이 있으므로 이벤트 객체를 통해
        반드시 preventDefault()함수를 사용해야 한다. */}
      <a href="/aTag" onClick={(e) => {e.preventDefault();}}>A태그</a> &nbsp;
      {/* Link 컴포넌트는 NavLink와 기능은 동일하지만, active 속성값이 부여되지 않는다. */}
      <Link to='/linkTag'>Link컴포넌트</Link>
    </nav>
  </>);
}

/* Outlet 컴포넌트 : 웹사이트 제작시 공통으로 사용되는 레이아웃에서 특정 요청에 따른
    내용만 변경해야 할때 사용한다. */
const CommonLayout = () => {
  return (<>
...
/* localhost/intro/router 경로가 요청되었을때 Outlet 컴포넌트 
부분에 삽입되어 렌더링된다. */
...
function App() {
  return (<>
    {/* 라우팅 처리가 필요없는 컴포넌트는 전체페이지에서 공통으로 렌더링되는 네비게이션을
      주로 사용한다. */}
    <TopNavi />
    {/* 라우팅 처리가 필요한 컴포넌트는 아래와 같이 path, element라는 속성을 통해 
      경로와 렌더링 할 컴포넌트를 지정한다. */}
    <Routes>
      ...
      <Route path='/intro' element={<CommonLayout />}>
        ...
        {/* '/intro/router'와 같이 하위경로로 요청이 들어오면 RouterHooks컴포넌트를
          렌더링한다. */}
