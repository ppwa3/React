...
useEffect
  ...
  즉 화면에 기본 UI(밑그림)가 그려진 후 데이터를 얻어와서 채우는 방식으로
  동작한다.
*/
...
function LifeGood(props){
  ...
  */
  useEffect(function(){
    ...
    return ()=>{
      console.log('#Life','useEffect실행==>4.컴포넌트 인마운트');
    }
  // }); //1.의존성배열(두번째 인수) 없음
  //} , []); //2.의존성배열에 빈 배열을 할당
...
1.2개의 버튼을 누를때 마다 useEffect가 실행된다. 즉 리렌더링이 될때마다
  실행된다.
2.최초 실행시에만 useEffect가 실행되고, 그 이후에는 실행되지 않는다.
3.myCount가 변경될때마다 useEffect가 실행된다.
*/
